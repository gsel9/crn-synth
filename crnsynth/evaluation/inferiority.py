...
def estimate_equivalence_margin(data: pd.DataFrame, fraction=0.5):
    """In non-inferiority testing, a common practice is to set the margin value to a fraction
    of the lower limit of a confidence interval of the difference between the current therapy and
    the placebo obtained from a meta- analysis.

    When the outcome is mortality, the FDA has suggested a value of f of 0.50.

    The noninferiority margin is typically defined in terms of some fraction of the standard treatment
    effect to be preserved.
    """

    # 1. estimate CI for odds_ratio between placebo and standard treatment (active control)
    # 2. d_max = lower

    return  # d_max ** (1 - fraction)
